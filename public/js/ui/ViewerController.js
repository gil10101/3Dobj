...
class ViewerController {
  ...
  showShape(shapeName) {
    try {
      console.log(`ViewerController: Showing shape ${shapeName}`);
      ...
      console.log(`ViewerController: Found preset for ${shapeName}:`, preset);
      
      const geometryOptions = { ...preset.geometry };
      const materialOptions = { ...preset.material, wireframe: this.state.wireframe };
      
      console.log(`ViewerController: Creating geometry with options:`, geometryOptions);
      console.log(`ViewerController: Creating material with options:`, materialOptions);
      
      // Create geometry directly using the appropriate method
      let geometry;
      switch (shapeName.toLowerCase()) {
        case 'cube':
        case 'box':
          geometry = this.geometryFactory.createCube(geometryOptions);
          break;
        case 'sphere':
          geometry = this.geometryFactory.createSphere(geometryOptions);
          break;
        case 'torus':
          geometry = this.geometryFactory.createTorus(geometryOptions);
          break;
        case 'cylinder':
          geometry = this.geometryFactory.createCylinder(geometryOptions);
          break;
        case 'cone':
          geometry = this.geometryFactory.createCone(geometryOptions);
          break;
        default:
          throw new Error(`Unknown geometry type: ${shapeName}`);
      }
      
      console.log(`ViewerController: Created geometry:`, geometry);
      
      // Create material
      const material = this.geometryFactory.createMaterial(materialOptions);
      console.log(`ViewerController: Created material:`, material);
      
      // Use renderer's addObject method for proper management
      const mesh = this.renderer.addObject(geometry, material, {
        castShadow: true,
        receiveShadow: true
      });
      
      console.log(`ViewerController: Added object to scene:`, mesh);
      console.log(`ViewerController: Scene children count:`, this.renderer.scene.children.length);
      
      this.state.currentShape = shapeName;
      this.updateUI(shapeName);
      this.emit('shapeChanged', { shape: shapeName, mesh: mesh });
