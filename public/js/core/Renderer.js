...
class Renderer {
   ...
   */
  createScene() {
    console.log('Renderer: Creating scene...');
    this.scene = new THREE.Scene();
    this.scene.background = new THREE.Color(0x111111);
    console.log('Renderer: Scene created:', this.scene);
  }
  ...
  createCamera() {
    ...
    console.log('Renderer: Camera created:', this.camera);
  ...
  addObject(geometry, material, options = {}) {
    console.log('Renderer: Adding object to scene');
    console.log('Renderer: Geometry:', geometry);
    console.log('Renderer: Material:', material);
    console.log('Renderer: Options:', options);
    
    this.removeCurrentObject();
    
    const mesh = new THREE.Mesh(geometry, material);
    console.log('Renderer: Created mesh:', mesh);
    ...
    console.log('Renderer: Adding mesh to scene. Scene before:', this.scene.children.length);
    this.scene.add(mesh);
    console.log('Renderer: Scene after adding mesh:', this.scene.children.length);
    
    this.currentObject = mesh;
    this.emit('objectAdded', mesh);
    
    console.log('Renderer: Object added successfully. Current object:', this.currentObject);
  ...
  render() {
    if (!this.state.initialized) {
      console.log('Renderer: Not initialized, skipping render');
      return;
    }
    ...
    if (Math.random() < 0.001) { // Log roughly once every 1000 frames
      ...
      console.log('Renderer: Current object:', this.currentObject);
      console.log('Renderer: Auto-rotate:', this.state.autoRotate);
    }
  }
